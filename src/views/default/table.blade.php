                          ...
                          <div class='col-sm-3'>
                            ...
                            @else
                              <div class="form-check" style="margin-top : 4%; text-align: right;">
                                ...
                                @else
                                  @if(CRUDBooster::getValueFilter($col["field_with"]) == 'empty')
                                    <input name='filter_column[{{$col["field_with"]}}][type]' data-type='{{$col["type_data"]}}' class="filter-combo" type="checkbox" {{ (CRUDBooster::getTypeFilter($col["field_with"]) == 'empty')?"checked":"" }} value='empty'>
                                  @else
                            ...
                            <div class='row between-group' style="{{ (CRUDBooster::getTypeFilter($col["field_with"]) == 'between')?"display:block":"display:none" }}">
                              <div class='col-sm-6'>
                                <div class='input-group'>
                                  ...
                                  type='text' 
                                  class='filter-value-between form-control {{ (in_array($col["type_data"],["date","time","datetime","timestamp", "varchar"])||(strpos(strtolower($col['label']),"date")!==false))?"datepicker":"" }}' readonly placeholder='{{$col["label"]}} {{trans("crudbooster.filter_from")}}' name='filter_column[{{$col["field_with"]}}][value][]' value='<?php
                              ...
                              <div class='col-sm-6'>
                                <div class='input-group'>
                                  ...
                                  {{ (CRUDBooster::getTypeFilter($col["field_with"]) != 'between')?"disabled":"" }}
                                  type='text' 
                                  class='filter-value-between form-control {{ (in_array($col["type_data"],["date","time","datetime","timestamp", "varchar"])||(strpos(strtolower($col['label']),"date")!==false))?"datepicker":"" }}' readonly placeholder='{{$col["label"]}} {{trans("crudbooster.filter_to")}}' name='filter_column[{{$col["field_with"]}}][value][]' value='<?php
                                  $value = CRUDBooster::getValueFilter($col["field_with"]); 
